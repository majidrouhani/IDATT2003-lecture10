package edu.ntnu.idatt2001.mvc.contacts.views;
...
/**
 * A dialog used to get the necessary information about a contact from the user,
 * in order to be able to create a ContactDetails instance to be added to the
 ...
 * @version 2020-03-16
 */
public class ContactDetailsDialog extends Dialog<Contact> {

  /**
   * The mode of the dialog. If the dialog is opened to edit an existing Contact,
   * the mode is set to <code>Mode.EDIT</code>. If the dialog is opened to create
   * a new contact, the <code>Mode.NEW</code> is used.
   */
  public enum Mode {
    NEW, EDIT, INFO
  }
  ...
  /**
   * Creates an instance of the ContactDetailsDialog to get information to create
   * a new instance of ContactDetails.
   */
   ...
   * @param editable if set to <code>true</code>, the dialog will enable editing
   *                 of the fields in the dialog. if <code>false</code> the
   *                 information will be displayed in non-editable fields.
   */
  public ContactDetailsDialog(Contact contact, boolean editable) {
    super();
    if (editable) {
      this.mode = Mode.EDIT;
    } else {
  ...
  private void createContent() {
    ...
    // Check out:
    // https://docs.oracle.com/javase/8/javafx/api/javafx/scene/control/Dialog.html#setResultConverter-javafx.util.Callback-
    ...
    setResultConverter((ButtonType button) -> {
      ...
      if (button == ButtonType.OK) {
        ...
        if (mode == Mode.NEW) {
          result = new Contact(name.getText(), phoneNumber.getText(), address.getText());
